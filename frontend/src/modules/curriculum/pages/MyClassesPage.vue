...
<template>
  ...
  <div v-if="isManager && !selectedTeacherId && !loading" class="card">
  ...
  <div v-if="!loading && assignments.length > 0" class="metric-grid-lg">
    <div
      v-for="a in assignments"
      :key="a.id"
      class="card p-5 transition-shadow hover:shadow-md"
    >
      <h3 class="text-base font-semibold text-md-text">{{ getClassLabel(a) }}</h3>
      <p class="mt-1 text-[0.875rem] text-md-text-secondary">{{ getSubjectLabel(a) }}</p>
