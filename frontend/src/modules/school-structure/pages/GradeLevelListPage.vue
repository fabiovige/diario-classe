...
async function loadData() {
  ...
  try {
    ...
    const response = await schoolStructureService.getGradeLevels(params)
...
function handleDelete(gradeLevel: GradeLevel) {
  confirmDelete(async () => {
    try {
      await schoolStructureService.deleteGradeLevel(gradeLevel.id)
      toast.success('Nivel de ensino excluido')
      loadData()
    } catch {
      toast.error('Erro ao excluir nivel de ensino')
    }
  })
}
